...
Local_folder <- "../Input/1.Sequencing_runs_output"
...
occ.results <- read_csv("../Input/Occ.fate.csv")
...
occ.results %>% 
  left_join(ASV.nested %>% 
              ...
              group_by(Hash) %>% 
              summarise (tot = sum(nReads))) %>% 
  ggplot(aes(x = cut_interval(real, n = 20))) +
...
output <- "../Output/Hash_Key_all_together.csv"
