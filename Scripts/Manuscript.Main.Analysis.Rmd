...
#For example, we expect the Hood Canal to be T $>$ 12\degree C and pH $<$ {7.9} -- where community 3 is the most likely community -- more than four times as often in 2095 than in 2017 (4.9 \% vs 20.7\%) )
...
In general, the prevalence of cluster 3 in the Hood Canal will increase from 54.2% of the time to 75% of the time,
```{r echo = F}
...
#For example, in in the Hood Canal at present, we expect surface waters to have pH $<$ 7.9 and T $>$ 19$\degree$C only 1\% of the time. In 2095, we expect these conditions 6 times more frequently (i.e., 6\% of the time).
...
richness.by.phylum.by.point %>% 
  ...
  summarise(condition = sum(Temperature > 19 & pH < 7.9),
            totalcount = n()) %>% 
  mutate(percent_time = condition / totalcount)
...
# Responses in diatoms are not all in the same direction - Pseudonitzschia spp shows higher probability of presence at low pH values, and low pH values are more common in the 2095 scenario
