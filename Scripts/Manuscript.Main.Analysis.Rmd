...
Using a global ANOVA to see if there are differences between Years (2095-2017) in both Areas. Then a  wilcoxon test to see if there is a difference  on each phylum
```{r}

## GLOBAL TEST

input.for.left %>% 
  mutate(sim.row = rep(1:10873, n_distinct(taxa))) -> input.richness

input.richness %>% 
  nest(-Area) %>% 
  mutate(data = map (data, ~ .x %>% 
                       mutate(present = case_when(present < 0.5 ~ 0,
                                                  TRUE          ~ 1)) %>% 
                         group_by(Year,  sim.row) %>% 
                         summarise( Richness = sum(present)) ),
         wilcoxon = map(data, ~ wilcox.test(Richness ~ Year, data = .x, alternative = "greater")),
...
input.richness %>% 
  ...
  #  slice(1) %>%
    mutate(data = map (data, ~ .x %>% 
                         ...
                         group_by(Year,  sim.row) %>% 
                         summarise( Richness = sum(present)) )) %>% 
   mutate(wilcoxon = map (data, ~ wilcox.test(Richness ~ Year, data = .x, alternative = "greater") ))  -> results.lm
