...
#while the changes on the San Juan Island are undetectable (p $=$ 0.8).  
...
richness.by.point %>% 
  ...
  mutate(wilcoxon = map(data, ~ wilcox.test(richness ~ Year, data = .x)),
         tidy_wilcox = map_df(wilcoxon, broom::tidy)) 
...
richness.by.phylum.by.point %>% 
   nest(-phylum,-Area) %>% 
   mutate(wilcoxon = map (data, ~ wilcox.test(richness ~ Year, data = .x) ),
          wiloc.tidy= map(wilcoxon, broom::tidy))  -> results.lm
   

results.lm %>%  filter (phylum == "Bacillariophyta") %>% unnest(wiloc.tidy)

results.lm %>%  filter (phylum == "Dinophyceae") %>% unnest(wiloc.tidy)

results.lm %>% 
  unnest(wiloc.tidy) %>% 
  
  ggplot(aes(x = phylum , y = `p.value`)) +
