...
saveRDS( file = here("Output","Temp.pH.grid.RDS"))

future.scenario %>% 
  write_rds(file = here("Output", "future.simulation.rds"))
...
expanded.future$sd <- apply(posterior.predict.all.taxa,2,sd )
```
...
fig3.right["Dinophyceae"]
fig3.right["Bacillariophyta"]
...
paths <- paste0("Supp.fig.",names(fig3.left),".png")

pmap(.l = list(Suppl.Material.plots.final, paths),
     .f = function(a,b) a +  ggsave(filename = here("Figures","Supplementary", b),
                                    device = "png",
                                    units = "in",
                                    dpi = "retina",
                                    width = 20,
